...
use paralegal_policy::{
    assert_error,
    paralegal_spdg::{Ctrl, DefKind, rustc_portable::DefId},
    Context, Marker, ControllerId
};
...
fn check(ctx: &mut Context) -> Result<()> {
   let found = ctx.controllers()
        .collect::<Vec<_>>()
        .into_iter()
        .any(|(deleter_id, deleter)| {
            ctx.marked_type(marker!(user_data)).collect::<Vec<_>>().into_iter().all(|t| {
                ...
                deleter
                    ...
                    .any(|sink| {
                        sources.iter()
                            .any(|src| 
                                ctx.flows_to(deleter_id, src, sink)
                            )
                    })
            })
        });
...
fn main() -> Result<()> {
    ...
    cmd.get_command().args(["--", "-p", "plume-models"]);
    cmd
        .run(dir)?
        .with_context(check)
}
