...
impl <'tcx, A: JoinSemiLattice> Value<'tcx, A> {
    pub fn override_(&mut self, projections: &ty::List<mir::PlaceElem>, dep: A) -> Option<HashSet<mir::Place<'tcx>>> {
        ...
        match (self, other) {
            ...
            (Abstract(my), other) => {
                ...
                *self = other.deep_clone();
                match self {
                    Tuple(_, general) | Struct(_, general) => general.get_or_insert_with(A::default).join(&moved),
                    _ => unreachable!(),
                };
                changed = true;
            }
            (Uninit, _) => { *self = other.deep_clone(); changed = true }
            (Ref(refs), Ref(others)) => others.iter().for_each(|r| changed |= refs.insert(*r)),
            (Struct(fields, val), Struct(other_fields, other_val)) => changed |= join_structure(fields, val, other_fields, other_val),
            (Tuple(tfields, tval), Tuple(other_tfields, other_tval)) => changed |= join_structure(tfields, tval, other_tfields, other_tval),
...
impl <'tcx, A: JoinSemiLattice + Default> JoinSemiLattice for Memory<'tcx, A> {
    fn join(&mut self, other: &Self) -> bool {
        ...
        self.mem.iter_mut().zip(other.mem.iter()).for_each(|(target, source)| has_changed |= target.join(source));
        has_changed
    }
}
