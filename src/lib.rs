...
impl ToForge for ProgramDescription {
    ...
    where A::Doc: Clone
    {
        ...
        macro_rules! lines {
            ($e:expr) => {
                alloc.intersperse($e, alloc.hardline())  
            };
        }
        
        fn make_one_sig<'b, A: pretty::DocAllocator<'b, ()>, I: ToForge>(alloc: &'b A, i: &'b I, parent: &'static str) -> pretty::DocBuilder<'b, A, ()> 
            where A::Doc : Clone
        {
        ...
        lines!([
            ...
            alloc.nil(),
            lines!(self.all_arguments().into_iter().map(|a| make_one_sig(alloc, a, ARG_NAME))),
            alloc.nil(),
            lines!(self.all_sinks().into_iter().map(|s| make_one_sig(alloc, s, FN_NAME))),
            alloc.nil(),
            lines!(self.d.keys().map(|e| make_one_sig(alloc, e, CTRL_NAME))),
...
impl<'tcx, 'p> Visitor<'tcx, 'p> {
    ...
    fn analyze(&mut self) -> std::io::Result<ProgramDescription> {
        ...
        }).collect::<std::io::Result<HashMap<Endpoint,Ctrl>>>().map(|d| ProgramDescription { d })
    }
}
