...
impl std::fmt::Display for ReadCommandErr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            TooManyArgs(num, full) => write!(f, "too many arguments provided, expected {num}. Full command {full}"),
    ...
    fn run_command(&self, command: Command) -> Option<()> {
        match command {
            Command::Paths(from, to) => {
                ...
                for path in petgraph::algo::all_simple_paths::<Vec<_>, _>(&self.graph, from, to, 0, None) {
                    println!("{}", Print(|fmt| {
    ...
    dfpp::rust::rustc_span::create_default_session_if_not_set_then(|_| {
        ...
        loop {
            match prompt.interact_text() {
                Ok(cmd) => if let Some(()) = repl.run_command(cmd) {
