...
impl ProgramDescription {
    ...
    pub fn canonical_read(path: impl AsRef<Path>) -> Result<Self> {
        ...
        if magic != ser_magic {
            anyhow::bail!("Magic number mismatch: Expected {ser_magic:x}, got {magic:x}. Likely this application was compiled against a different version of the paralegal-pdg library then used by the flow analyzer.");
        }
        cfg_if! {
            if #[cfg(feature = "binenc")] {
...
paralegal_utils::fs_storable_default!(AnalyzerStats);
