...
//!    The [`::global()`](SPDGGenCommand::global) method uses `cargo parable` for
//!    this purpose, e.g. a global installation of `cargo-parable` that was
...
pub use paragraph;
use paragraph::ProgramDescription;
use std::{
    fs::File,
    path::{Path, PathBuf},
    process::Command,
...
/// Configuration of the `cargo parable` command.
///
...
impl SPDGGenCommand {
    /// Use a global installation of `parable` via `cargo parable`.
    pub fn global() -> Self {
...
/// parable` by hand with [`Self::custom`].
...
impl GraphLocation {
    ...
    pub fn std(dir: impl AsRef<Path>) -> Self {
        Self(dir.as_ref().join(paragraph::FLOW_GRAPH_OUT_NAME))
    }
