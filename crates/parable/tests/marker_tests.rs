...
use parable::{define_flow_test_template, test_utils::*};

const TEST_CRATE_NAME: &str = "tests/marker-tests";

lazy_static! {
    static ref TEST_CRATE_ANALYZED: bool =
        run_parable_with_flow_graph_dump_and(TEST_CRATE_NAME, ["--no-cross-function-analysis"]);
}
...
define_test!(use_wrapper: ctrl -> {
    ...
    assert!(ctrl.types_for(&parable::desc::DataSource::FunctionCall(cs.call_site().clone()))
