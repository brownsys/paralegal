...
/// Run parable in the current directory, passing the
...
pub fn run_parable_with_graph_dump(dir: impl AsRef<Path>) -> bool {
    run_parable_with_graph_dump_and::<_, &str>(dir, [])
}

/// Crates a basic invocation of `cargo parable`, ensuring that the `cargo-parable`
/// and `parable` executables that were built from this project are (first) in the
/// `PATH`.
pub fn parable_command(dir: impl AsRef<Path>) -> std::process::Command {
    let path = std::env::var("PATH").unwrap_or_else(|_| Default::default());
    // Cargo gives us the path where it wrote `cargo-parable` to
    let cargo_parable_path = Path::new("../../target/debug/cargo-parable")
        .canonicalize()
        .unwrap();
    let mut new_path =
        std::ffi::OsString::with_capacity(path.len() + cargo_parable_path.as_os_str().len() + 1);
    // We then append the parent (e.g. its directory) to the search path. THat
    // directory (we presume) contains both `parable` and `cargo-parable`.
    new_path.push(cargo_parable_path.parent().unwrap_or_else(|| {
        panic!(
            "cargo-parable path {} had no parent",
            cargo_parable_path.display()
        )
    }));
    new_path.push(":");
    new_path.push(path);
    let mut cmd = std::process::Command::new(cargo_parable_path);
    cmd.arg("parable").env("PATH", new_path).current_dir(dir);
...
/// [`read_non_transitive_graph_and_body`](crate::dbg::read_non_transitive_graph_and_body).
///
/// Allows for additional arguments to be passed to parable
pub fn run_parable_with_graph_dump_and<I, S>(dir: impl AsRef<Path>, extra: I) -> bool
where
...
pub fn run_parable_with_flow_graph_dump(dir: impl AsRef<Path>) -> bool {
    run_parable_with_flow_graph_dump_and::<_, &str>(dir, [])
}
...
///
/// The result is suitable for reading with [`PreFrg::from_file_at`]
pub fn run_parable_with_flow_graph_dump_and<I, S>(dir: impl AsRef<Path>, extra: I) -> bool
where
    I: IntoIterator<Item = S>,
    S: AsRef<std::ffi::OsStr>,
{
