...
//! being able to reference the same code in the two executables `parable` and
//! `cargo-parable` (a structure suggested by [rustc_plugin]).
#![feature(
    rustc_private,
    min_specialization,
    drain_filter,
...
/// configuration of parable [`struct@Args`] which is stored in `args`. `cargo_args` is
...
struct ArgWrapper {
    /// This argument doesn't do anything, but when cargo invokes `cargo-parable`
    /// it always provides "parable" as the first argument and since we parse with
