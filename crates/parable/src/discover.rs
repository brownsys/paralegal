...
//!
//! Items of interest is anything annotated with one of the `parable::`
//! annotations.
...
pub struct CollectingVisitor<'tcx> {
    ...
    /// Functions that are annotated with `#[parable::analyze]`. For these we will
...
impl<'tcx> CollectingVisitor<'tcx> {
    ...
    /// Does the function named by this id have the `parable::analyze` annotation
