...
use crate::{frg::{ForgeConverter, call_site_to_string}, utils::outfile_pls};
    ...
    fn after_expansion<'tcx>(
        ...
        queries
            ...
            .enter(|tcx| {
                ...
                let doc: DocBuilder<_, ()> = ForgeConverter::new(desc, tcx).serialize_forge(&doc_alloc, self.opts.modelctrl());
                ...
                let info = AdditionalInfo {
                    call_sites: desc.all_call_sites().into_iter().map(|cs| (call_site_to_string(tcx, cs), cs.clone())).collect()
                };
