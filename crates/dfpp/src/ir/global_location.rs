...
pub use crate::desc::{GlobalLocationS, RawGlobalLocation};
...
impl<'g> std::fmt::Display for GlobalLocation<'g> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        dfgraph::format_global_location(self.as_slice().iter(), f)
    }
}
...
pub trait CallSiteExt {
    fn new<L: IsGlobalLocation>(loc: &L, function: DefId, tcx: TyCtxt<'_>) -> Self;
}
