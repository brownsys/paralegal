...
// This is the one exception: if a clause introduces a variable that gets referred to more than once,
// we cannot use a NodeCluster. This is because clauses are meant to introduce a ~single~ object that can be referred to multiple times;
...
pub fn override_initialization_typ(
    intro: &VariableIntro,
    body: &ASTNode,
) -> Option<InitializationType> {
